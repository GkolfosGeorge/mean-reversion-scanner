...
def run_backtest(
    ...
    # None (default) = old behavior, no sector cap in the backtest.
    ...
    # pass) exceeds max_pairwise_correlation. None (default) = filter OFF.
    ...
    if sectors is not None:
        ...
        print(f"   Sector diversification: INACTIVE (sectors=None — no cap applied)")
    ...
    for i, review_date in enumerate(monthly_dates):
        ...
        if slots_available > 0 and deployable > 100 and entry_allowed:
            ...
            def _recent_returns(ticker: str):
                ...
                try:
                    # Reuses the same clean_data cache as the rest of the loop
                    # (Performance note above) instead of re-slicing `data`.
                    closes = clean_data[ticker]["Close"] if ticker in clean_data else data[ticker]["Close"].dropna()
            ...
            if candidates:
                ...
                for ticker, result in candidates:
                    ...
                    # and applied the SAME way regardless of sizing_method below.
    ...
    # no longer computes Sharpe/Sortino/win-rate/etc. inline. See
    # backtest_metrics.py for exact formulas (Sharpe/Sortino/Calmar now use
    # equity-curve periodic returns instead of trade-level pnl_pct — numbers
    # will differ slightly, and more accurately, from older runs).
...
def print_backtest_report(results: dict) -> None:
    ...
    if s.get("sector_cap_active"):
        print(f"  Sector cap: max {s.get('max_per_sector')} positions/sector")
    if s.get("correlation_filter_active"):
        print(f"  Correlation filter: max |corr|={s.get('max_pairwise_correlation'):.2f}, "
              f"lookback={s.get('correlation_lookback_days')}d")
    print(f"{'═'*60}")
